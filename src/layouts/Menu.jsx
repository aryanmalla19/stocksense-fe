...
const menuItems = [
  {
    ...
    items: [
      ...
      { icon: <FaShoppingCart />, label: "Trade Stocks", href: "/buysell" },
      { icon: <FaMoneyCheckAlt />, label: "Watchlist", href: "/watchlists" },
      { icon: <FaUserAlt />, label: "Portfolio", href: "/portfolio" },
      { icon: <FaMoneyCheckAlt />, label: "IPO Applications", href: "/shares" },
...
const Menu = () => {
  ...
  return (
    <div
      className={`text-lg mt-5 flex flex-col min-h-screen gap-30 ${
        ...
        <div className="flex flex-col gap-2 " key={category.title}>
          ...
          {category.items.map((item) => (
            <Link
              to={item.href}
              key={item.label}
              className={`flex items-center w-full justify-center lg:justify-start gap-4 py-2 px-4 rounded transition-colors duration-300
                ${
                  theme === "dark"
            ...
            >
              <span className="w-5 text-xl h-5">{item.icon}</span>
